...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="group relative bg-white border border-slate-200 overflow-hidden flex flex-col justify-between hover:border-slate-900 transition-colors">
      ...
      <div className="relative aspect-square bg-slate-50 border-b border-slate-200 overflow-hidden">
        <img
        ...
        <div className="space-y-2 pt-2 border-t border-slate-200">
          ...
          {profile && (
            <button
              ...
              className={`w-full py-1.5 px-2 text-xs font-bold flex items-center justify-center space-x-1.5 border transition-colors uppercase cursor-pointer ${
                isLiked
            ...
            >
              <Heart
                className={`w-3.5 h-3.5 ${isLiked ? "fill-current text-white" : "text-slate-500"}`}
              />
              <span>{isLiked ? "In Wishlist" : "Add to list"}</span>
            </button>
          )}
        </div>
      </div>
